...
const reducer = (state: any, action: any) => {
  switch (action.type) {
    ...
    //   return {
    //     ...state,
    //     items: state.items.map((item: any, idx: number) => {
    ...
    //       (_: any, idx: number) => idx !== action.payload.index
    //     ),
    //   };
    // default:
    //   return state;
...
const ShoppingList = (props: any) => {
  ...
  const onChangeShoppingListItemName = (e: any) => {
    dispatch({
    ...
    <StyledContainer>
      <StyledWrapper>
        ...
        <div style={{ marginBottom: "1.5rem" }}>
          {shoppingList.items.map((item: any, index: any) => (
            <ShoppingListRow
              key={item.id}
              item={item}
              index={index}
